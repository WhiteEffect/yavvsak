...
if(client.ping > 2500) {
           ...
           let sChannel = msg.guild.channels.find(c => c.name === "🍅・saldırı")
...
client.on('ready', () => {
    ...
    var interval = setInterval (function () {
        var channel = client.channels.find(c => c.id === "633346281376448595")
        request(`https://mcapi.xdefcon.com/server/play.atinanw.com/full/json`, function (error, response, body) {
            body = JSON.parse(body);
            if(body.serverStatus === "online"){
                client.user.setGame(`${body.players} kişi oynuyor. a!yardım ● ${client.users.size} kullanıcıyı izliyor.`, "https://www.twitch.tv/atinanw")
            } else {
                client.user.setGame(`a!yardım ● ${client.users.size} kullanıcıyı izliyor.`, "https://www.twitch.tv/atinanw");
            }
        })
    }, 3000);
...
client.on("ready", () => {
  var interval = setInterval(function() {
    var channel = client.channels.find(c => c.id === "686128532274937884");
      channel.setName(`▪ Tarih: ${moment().format("DD/MM/YYYY")}`);
        } 
    );
  }, 60000);
...
client.on('ready', () => {
    ...
    var interval = setInterval (function () {
        var channel = client.channels.find(c => c.id === "686128032548782120")
        request(`https://mcapi.xdefcon.com/server/play.atinanw.com/full/json`, function (error, response, body) {
            body = JSON.parse(body);
...
client.on("ready", () => {
  ...
  var interval = setInterval(function() {
    var channel = client.channels.find(c => c.id === "686128604266233856");
        request(`https://mcapi.xdefcon.com/server/play.atinanw.com/full/json`, function (error, response, body) {
        body = JSON.parse(body);
        if (body.serverStatus === "online") {
                channel.setName(`▪ Gecikme: ${body.ping}`);
            } else {
                channel.setName("▪ Gecikme: 0");
        }
      }
    );
  }, 1000);
...
client.on("ready", () => {
  ...
  var interval = setInterval(function() {
    var channel = client.channels.find(c => c.id === "686127497858777098");
    request(
      `https://mcapi.xdefcon.com/server/play.atinanw.com/full/json`,
      function(error, response, body) {
        body = JSON.parse(body);
        if (body.serverStatus === "online") {
          channel.setName(`Sunucu Bilgisi ${body.players}-${body.maxplayers}`);
        } else {
          channel.setName("Durum: Bakımda");
        }
      }
...
client.on('guildMemberAdd', member => {
  ...
  .setColor('GREEN')
  .setTitle('**GİRİŞ**')
  .addField("──────────────────────────", `📥 | ${member.user.username} sunucuya giriş yaptı.`)   
  .addField("──────────────────────────", `🍵 Tarih: ${moment().format("DD/MM/YYYY")}`) 
  .setThumbnail('https://cdn.discordapp.com/emojis/511644642492547084.gif?v=1')
  .setTimestamp()
...
client.on('guildMemberRemove', member => {
  ...
  .setColor('RED')
  .setTitle('**ÇIKIŞ**')
  .addField("──────────────────────────", `📥 | ${member.user.username} sunucudan çıkış yaptı.`)   
  .addField("──────────────────────────", `🍵 Tarih: ${moment().format("DD/MM/YYYY")}`) 
  .setThumbnail('https://cdn.discordapp.com/emojis/511644642492547084.gif?v=1')
  .setTimestamp()
  .setFooter(member.user.username, member.user.avatarURL)
